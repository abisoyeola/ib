...
namespace POS_SALES
{
    public partial class Supplier_Inventory : Form
    {
        ...
        private void Customer_Inventory_Activated(object sender, EventArgs e)
        {
            if (this.label2.Text == "0")
            {
                ...
                {
                    code = null;
                    this.gclass.DGVTable("SELECT id as 'S/N',vn as 'SUPPIER NAME',phone as 'PHONE NO',add1 as 'ADDRESS',bal as 'SUPPLIER BALANCE' FROM vendor", this.dataGridView1);
        ...
        {
            code = this.dataGridView1.SelectedRows[0].Cells[0].Value.ToString();
            balance = this.dataGridView1.SelectedRows[0].Cells[4].Value.ToString();
            name = this.dataGridView1.SelectedRows[0].Cells[1].Value.ToString();
            this.label2.Text = "0";
            new Supplier_Payment().ShowDialog();
        }
