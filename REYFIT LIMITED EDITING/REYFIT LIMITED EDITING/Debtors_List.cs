...
namespace POS_SALES
{
    public partial class Debtors_List : Form
    {
        ...
        private void Customer_Inventory_Activated(object sender, EventArgs e)
        {
            if (this.label2.Text == "0")
            {
                try
                {
                    ...
                    {
                        tot_debit += Convert.ToDecimal(dataGridView1.Rows[i].Cells[6].Value);
                    ...
                    total_debits.Text = tot_debit.ToString();
                    this.total_debits.Text = $"{Convert.ToDecimal(this.total_debits.Text):N}";
                }
