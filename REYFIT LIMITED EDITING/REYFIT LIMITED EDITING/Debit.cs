    ...
    public partial class Debit : Form
    {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string str2 = (Convert.ToInt32(new MySqlCommand("SELECT COUNT(*) FROM customer_account", this.gclass.conn()).ExecuteScalar().ToString()) + 1).ToString();
                ...
                MySqlCommand jh = new MySqlCommand("insert into customer_account(pdate,customerid,customer_name,paymentno,entry_type,debit,balance,postedby)VALUES('" + DateTime.Now.ToString("yyyy/MM/dd") + "','" + customer_id.Text + "','" + customer_name.Text + "','" + code.ToString() + "','" + Program.DEBIT + "','" + txtAmtpaid.Text + "','" + lblBalance.Text + "','" + Program.user + "')", cn);
                jh.ExecuteNonQuery();
                this.gclass.INSERT2("UPDATE customer_card SET balance = '" + this.lblBalance.Text + "' WHERE customerid = '" + this.customer_id.Text + "' ");
                this.gclass.success("Custmer Account Updated Successfully");
                ...
                listBox1.Items.Add("====================================================================");
                listBox1.Items.Add("DEBITED RECIEPT");
                ...
                listBox1.Items.Add("Amount Debited:  " + txtAmtpaid.Text);
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            try
            {
                this.lblOutstandingbalance.Text = Customer_Inventory.balance.ToString();
                this.customer_id.Text = Customer_Inventory.code.ToString();
                this.customer_name.Text = Customer_Inventory.name.ToString();
            }
            catch (Exception exception)
            {
                this.gclass.error(exception.Message.ToString());
       ...
       private void button6_Click(object sender, EventArgs e)
        ...
        private void txtAmtpaid_TextChanged_1(object sender, EventArgs e)
        {
            try
            {
                this.lblBalance.Text = $"{Convert.ToDecimal(this.txtAmtpaid.Text) + Convert.ToDecimal(this.lblOutstandingbalance.Text):N}";
            }
            catch (Exception)
            {
                ...
                this.lblBalance.Text = (Convert.ToDecimal(this.txtAmtpaid.Text) + Convert.ToDecimal(this.lblOutstandingbalance.Text)).ToString();
            }
        }
